...
class BaseUnit(Sprite):
    """
    This is the basic unit spirte class used to run all the units in our game.
    The following Unit Base was developed using code from assignment 4 and 
    pygame. 

    The following class will have: (1) the basic information about
    the unit (2) and relevant functions to determine the unit's actions 
    like attacks.

    
    The base images will be set in the subclasses since the image will be rendered
    using a tilesheet in the game.
    """
    ...
    def get_unit_at_pos(pos):
        ...
        is present.
        """
        for unit in BaseUnit.active_units:
            if (unit.tile_x, unit.tile_y) == pos:
                return u
    ...
    def get_defense(self, equip = None):
        """
        Returns this unit's defense. If there is equipment that grants bonuses
        include the bonus
        """
        if equip:
            return self.defense 
    ...
    def is_tile_in_range(self, from_tile, from_pos, to_pos):
        # Check to see what tile to run the game at?
        """
